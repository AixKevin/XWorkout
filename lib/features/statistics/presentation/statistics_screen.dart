...
import 'package:flutter/material.dart'
    show CircularProgressIndicator, Icons, Colors;
...
// Providers
final totalWorkoutsProvider = FutureProvider.autoDispose<int>(
    (ref) => ref.watch(statisticsRepositoryProvider).getTotalWorkouts());
final thisWeekWorkoutsProvider = FutureProvider.autoDispose<int>(
    (ref) => ref.watch(statisticsRepositoryProvider).getThisWeekWorkouts());
final currentStreakProvider = FutureProvider.autoDispose<int>(
    (ref) => ref.watch(statisticsRepositoryProvider).getCurrentStreak());
final volumeTrendProvider =
    FutureProvider.autoDispose<List<Map<String, dynamic>>>(
        (ref) => ref.watch(statisticsRepositoryProvider).getVolumeTrend(30));
final topExercisesProvider =
    FutureProvider.autoDispose<List<Map<String, dynamic>>>(
        (ref) => ref.watch(statisticsRepositoryProvider).getTopExercises(5));
final weeklyGoalProvider = FutureProvider.autoDispose<Map<String, dynamic>>(
    (ref) => ref
        .watch(statisticsRepositoryProvider)
        .getWeeklyGoalProgress(target: 3));

final exerciseStatsProvider = FutureProvider.autoDispose
    .family<Map<String, dynamic>, String>((ref, exerciseId) =>
        ref.watch(statisticsRepositoryProvider).getExerciseStats(exerciseId));
                            ...
                            data: (data) => data.isEmpty
                                ...
                                : _SimpleLineChart(
                                    data: data.map((row) {
                                      ...
                                      return {
                                        ...row,
                                        'volume': WeightUnitUtils.kgToDisplay(
                                            value, weightUnit),
                                      };
                                    }).toList(),
                                    yKey: 'volume',
...
class _TopExercisesList extends ConsumerWidget {
  ...
  void _showExerciseStats(
    ...
    showCupertinoModalPopup(
      context: context,
      builder: (context) => _ExerciseStatsSheet(
      ...
      LineChartData(
        gridData: FlGridData(
          ...
          getDrawingHorizontalLine: (value) =>
              FlLine(color: CupertinoColors.systemGrey6, strokeWidth: 1),
        ),
          ...
          Expanded(
            child: statsAsync.when(
              data: (data) {
                ...
                final maxWeightTrend =
                    data['maxWeightTrend'] as List<Map<String, dynamic>>;
                        ...
                        children: [
                          Expanded(
                            child: _StatBox(
                              title: '最大重量',
                              value:
                                  '${WeightUnitUtils.formatKgToDisplay(maxWeight, weightUnit)} $weightUnit',
                          ...
                          Expanded(
                            child: _StatBox(
                              title: '总容量',
                              value:
                                  '${(WeightUnitUtils.kgToDisplay(totalVolume, weightUnit) / 1000).toStringAsFixed(1)}k $weightUnit',
                              icon: Icons.layers,
                          ...
                          children: [
                            ...
                            Expanded(
                              child: _SimpleLineChart(
                                data: maxWeightTrend.map((row) {
                                  final value =
                                      (row['value'] as num?)?.toDouble() ?? 0;
                                  return {
                                    ...row,
                                    'value': WeightUnitUtils.kgToDisplay(
                                        value, weightUnit),
                                  };
                                }).toList(),
                                yKey: 'value',
